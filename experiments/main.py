...
if __name__ == '__main__':    
    ...
    parser.add_argument("--delta", type=float, help="The " + delta_letter + " budget of the DP", default=None, required=False)
    ...
    parser.add_argument("--seed", type=int, help="The seed for replicability", default =-1, required = False)
    ...
    if args.mechanism == "vanilla":
        start= time.time()
        greedy_rl = GreedyRLClassifier(min_support=0.0, max_length=args.max_length, verbosity=verbosity, max_card=args.max_card, allow_negations=True), seed = args.seed )
        greedy_rl.fit(X_unbias, y, features=features_unbias, prediction_name=prediction) 
        end=time.time() - start           
        
    elif args.mechanism == "Exponential":
        start= time.time()
        greedy_rl =  DPGreedyRLClassifier(min_support=0.0, max_length=max_length, verbosity=verbosity, max_card=max_card, allow_negations=True, epsilon = epsilon, delta = 0.0, seed = args.seed)
        greedy_rl.fit(X_unbias, y, features=features_unbias, prediction_name=prediction)       
        end=time.time() - start                        
        ...
        greedy_rl = DpSmoothGreedyRLClassifier(min_support=args.min_support, max_length=args.max_length, verbosity=verbosity, max_card=args.max_card, allow_negations=True, epsilon = args.epsilon, delta =args.delta, confidence = args.confidence, noise = noise, seed = args.seed)
        greedy_rl.fit(X_unbias, y, features=features_unbias, prediction_name=prediction)        
        ...
        start= time.time()
        noise = args.mechanism.split("-")[1]
        greedy_rl = DpNoiseGreedyRLClassifier(min_support=0.0, max_length=args.max_length, verbosity=verbosity, max_card=args.max_card, allow_negations=True, epsilon = args.epsilon, delta =args.delta, noise = noise, seed = args.seed) 
        greedy_rl.fit(X_unbias, y, features=features_unbias, prediction_name=prediction)  
        args.delta =greedy_rl.delta
        end=time.time() - start           
    ...
    print([args.dataset, args.mechanism, rformat(args.epsilon), pformat(args.delta, "e", 2), rformat(args.min_support), rformat(args.confidence), args.max_card, N, pformat(end), pformat(np.average(greedy_rl.predict(X_unbias) == y))])
