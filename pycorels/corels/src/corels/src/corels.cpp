...
void evaluate_children(CacheTree* tree, Node* parent, tracking_vector<unsigned short, DataStruct::Tree> parent_prefix,
    ...
    //std::cout << "Gmax_length = " << Gmax_length << ", parent->depth() = " << parent->depth() << std::endl;
    //std::cout << "Gmin_support_num = " << Gmin_support_num << std::endl;
    ...
    for (i = 1; i < nrules; i++) {
        ...
        // AJOUT JULIEN : min_support for each rule WITHIN THE BUILT RULE LIST
        if (num_captured < Gmin_support_num)
            continue;
    ...
    if (parent->num_children() == 0) {
        ...
        if (parent->depth() >= Gmax_length){
            std::cout << "Ignoring prefix with length " << parent->depth() << std::endl;
        }      
    }*/
}
 ...
 */
void bbound_begin(CacheTree* tree, Queue* q, int max_length, double min_support) {
    start = timestamp();
    num_iter = 0;
    ...
    Gmin_support_num = min_support * tree->nsamples();
}
